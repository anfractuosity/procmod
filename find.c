...
int main(int argc, char **argv) {
    ...
    //char *find = "\x00\x26\x00\x02\x00\x01\x00\x00\x2a\x0a\x00\xb7\x2a\x01\xb5\x04";//asciitou16("Pause/play"); //argv[2];
    
    char *find = "maize/ui/BotTest";
    char *replace = "";//asciitou16("Meow");//argv[3];
    int bytelength = strlen(find);
        ...
        while (1) {
            ...
            for (i = 0; i < 8; i++) {
                if ((tmp >> (i * 8 ) & 0xFF) == find[pos]) {
                    ...
                    if (/*find[pos] == 0 && find[pos-1] == 0*/ bytelength == pos) {
                        printf
                            ("-------------- FOUND\n");
